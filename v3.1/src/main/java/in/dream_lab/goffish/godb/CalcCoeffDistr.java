...
public class CalcCoeffDistr extends
AbstractSubgraphComputation<CalcCoeffDistrSubgraphState, MapValue, MapValue, Text, LongWritable, LongWritable, LongWritable> implements ISubgraphWrapup {
	
	 public static final Log LOG = LogFactory.getLog(CalcCoeffDistr.class);
	public CalcCoeffDistr(String initMsg) {
...
//		LOG.info("Subgraph Value:" + getSubgraph());
  ...
  public void compute(Iterable<IMessage<LongWritable,Text>> messages) {
  	...
  	if(getSuperstep()==2){
  	  for(IRemoteVertex<MapValue, MapValue, LongWritable, LongWritable, LongWritable> rv :getSubgraph().getRemoteVertices()){
  	    sendMessage(rv.getSubgraphId(), txtMessage);
  	  }
  	}
  	...
  	if(getSuperstep()==6){
  	  long count=0;
  	  for(Object o:getSubgraph().getLocalVertices()){
  	    count++;
  	  }
  	  LOG.info("Local Vertex Count:"+ count);
  	  
  	count=0;
        for(Object o:getSubgraph().getRemoteVertices()){
          count++;
        }
        LOG.info("Remote Vertex Count:"+ count);
  	  voteToHalt();
  	}

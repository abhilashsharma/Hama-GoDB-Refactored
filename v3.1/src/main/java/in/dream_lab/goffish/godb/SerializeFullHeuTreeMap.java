...
public class SerializeFullHeuTreeMap extends AbstractSubgraphComputation<pathDistrSubgraphState, MapWritable, MapWritable, Text, LongWritable, LongWritable, LongWritable>
implements ISubgraphWrapup{
	// Variables and DataStructures
	
	private PrintWriter writer;

	public SerializeFullHeuTreeMap(String initMsg){
	  argument=initMsg;
	}
	String argument=null;
	private HueristicsTreeMap hueristics = new HueristicsTreeMap(); 
...
//	private HashMap<Long,Long> remoteSubgraphMap; 
	...
	private void init(Iterable<IMessage<LongWritable, Text>> messageList){
		...
		try{
			...
			hueristics = (HueristicsTreeMap)ois.readObject();
			ois.close();
	...
	private void gatherHueristics(Iterable<IMessage<LongWritable, Text>> messageList) {
		hueristics = new HueristicsTreeMap();//Re initialize as it has to accummulate statistics from all partitions including its own.
		for(IMessage<LongWritable, Text> message:messageList) {
			try {
				HueristicsTreeMap _hueristics = (HueristicsTreeMap)deserializeObjectFromString(new String(message.getMessage().toString()));
				hueristics.numVertices += _hueristics.numVertices;
				hueristics.numEdges += _hueristics.numEdges;
				// gather vertex stats
				for(Map.Entry<String,TreeMap<String,vertexPredicateStats>> entry:_hueristics.vertexPredicateMap.entrySet()){
				...
				// gather edge stats
				for(Map.Entry<String,TreeMap<String,edgePredicateStats>> entry:_hueristics.edgePredicateMap.entrySet()){
		...
		for(Map.Entry<String,TreeMap<String,vertexPredicateStats>> entry:hueristics.vertexPredicateMap.entrySet()){
			for(Map.Entry<String, vertexPredicateStats> entry_inside: entry.getValue().entrySet()){
		...
		for(Map.Entry<String,TreeMap<String,edgePredicateStats>> entry:hueristics.edgePredicateMap.entrySet()){
			for(Map.Entry<String, edgePredicateStats> entry_inside: entry.getValue().entrySet()){

...
public class pathDistrHist extends
AbstractSubgraphComputation<pathDistrHistSubgraphState, MapWritable, MapWritable, Text, LongWritable, LongWritable, LongWritable>
implements ISubgraphWrapup{
	
	public pathDistrHist(String initMsg) {
		// TODO Auto-generated constructor stub
		Arguments=initMsg;
	}
	
	public static final Log LOG = LogFactory.getLog(pathDistrHist.class);
	...
	static HistogramHeuristic hueristics = new HistogramHeuristic(); 
...
//	        				ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForEdge(edge.getId());
			...
			hueristics=HistogramHueristicsLoad.getInstance();
				...
				LOG.info("Starting Query Execution");
				 queryEnd=false;
                                ...
                                // COMPUTE HUERISTIC BASED QUERY COST
                                {
                                        ...
                                        Double combinedCost=new Double(0);//FOR DEBUGGING
                                                        ...
                                                        while(It.hasNext()){
                                                                ...
                                                                // TODO: make cost not count in probability when no predicate on edge/vertex
                                                                {
                                                                        ...
                                                                        if ( currentStep.property == null )
                                                                                ...
                                                                                if ( hueristics.probabilityOfVertex(currentStep.property, currentStep.value.toString())!=-1  ){
...
//                                                                      }
                                                                        ...
                                                                        //avgRemoteDeg=0d;//FOR DEBUGGING..change later
                                                                        ...
                                                                        if(nextStep.property == null)
                                                                                ...
                                                                                if (hueristics.probabilityOfEdge(nextStep.property, nextStep.value.toString())!=-1  ) {
                                                                                        double edgeProbability=hueristics.probabilityOfEdge(nextStep.property, nextStep.value.toString());
                                                                                        vScanCost = eScanCost * edgeProbability;
                                                                                        networkCost = getSubgraph().getSubgraphValue().networkCoeff * prevScanCost * probability * avgRemoteDeg * edgeProbability;
                                                                                        resultSetNumber *= edgeProbability;
                                                                                        System.out.println("Edge:" + edgeProbability);
                                                                                }
                                                                                else {
                                                                                        totalCost = new Double(-1);
                                                                                        break;
...
//                                                        output(partition.getId(), subgraph.getId(), pos+":"+"for:"+String.valueOf(totalCost));
                                                }
                                                //reverse cost
                                                {
                                                        Double totalCost = new Double(0);
...
//                                                                      }
                                                        ...
                                                        joinCost *= resultSetNumber;
                                                        //System.out.println("Total Join Cost:" + joinCost);
...
//                                                        output(partition.getId(), subgraph.getId(), pos+":"+"rev:"+String.valueOf(totalCost));
                                                }
...
//                                                output(partition.getId(), subgraph.getId(), pos+":"+String.valueOf(queryCostHolder[pos]));
                                        }
...
//					System.out.println("Querying Output Path:" + vertexMessageStep.queryId+","+vertexMessageStep.startStep+","+true+","+vertexMessageStep.startVertexId );
				...
				IVertex<MapWritable,MapWritable,LongWritable,LongWritable> currentVertex = getSubgraph().getVertexById(new LongWritable(vertexMessageStep.vertexId));
				...
				if( nextStep.type == Type.EDGE ) {
					...
					if ( nextStep.direction == Direction.OUT ) {
						...
						if ( nextStep.property == null && nextStep.value == null ) {
							for( IEdge<MapWritable, LongWritable, LongWritable> edge: currentVertex.getOutEdges()) {
								//count++;
//								System.out.println("Traversing edges");
								IVertex<MapWritable,MapWritable,LongWritable,LongWritable> otherVertex = getSubgraph().getVertexById(edge.getSinkVertexId());
...
//									System.out.println("Path Till Now:" + _modifiedMessage.toString());
						...
						/* filtered edge*/
							...
							for( IEdge<MapWritable, LongWritable, LongWritable> edge: currentVertex.getOutEdges() ) {
								...
								if ( compareValuesUtil(edge.getValue().get(new Text(nextStep.property)).toString(), nextStep.value.toString()) ) {
									IVertex<MapWritable,MapWritable,LongWritable,LongWritable> otherVertex = getSubgraph().getVertexById(edge.getSinkVertexId());
									StringBuilder _modifiedMessage = new StringBuilder("");
									_modifiedMessage.append(vertexMessageStep.message).append("-->E:").append(String.valueOf(edge.getEdgeId().get())).append("-->V:").append(String.valueOf(otherVertex.getVertexId().get()));
									if ( !otherVertex.isRemote() ) {
...
//								output(partition.getId(), subgraph.getId(), currentVertex.getId()+":"+subgraphProperties.getValue("relation"));
							  //CHANGE it when moving to hashmaps
								if ( compareValuesUtil(edge.getValue().get(new Text(prevStep.property.toString())).toString(), prevStep.value.toString()) ) {
									IVertex<MapWritable,MapWritable,LongWritable,LongWritable> otherVertex = getSubgraph().getVertexById(edge.getSinkVertexId());
									StringBuilder _modifiedMessage = new StringBuilder("");
									_modifiedMessage.append("V:").append(String.valueOf(otherVertex.getVertexId().get())).append("<--E:").append(String.valueOf(edge.getEdgeId().get())).append("<--").append(vertexMessageStep.message);
									if ( !otherVertex.isRemote() ) {
...
//						ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForVertex(currentVertex.getId());
			...
			for(VertexMessageSteps stuff: getSubgraph().getSubgraphValue().forwardRemoteVertexList){
				...
				IRemoteVertex<MapWritable,MapWritable,LongWritable,LongWritable,LongWritable> remoteVertex = (IRemoteVertex<MapWritable, MapWritable, LongWritable, LongWritable, LongWritable>)getSubgraph().getVertexById(new LongWritable(stuff.vertexId));

...
public class pathDistrUndirected extends
AbstractSubgraphComputation<pathDistrUndirectedSubgraphState, MapValue, MapValue, Text, LongWritable, LongWritable, LongWritable> 
implements ISubgraphWrapup{
	
	public pathDistrUndirected(String initMsg) {
		// TODO Auto-generated constructor stub
		Arguments=initMsg;
	}
	
	public static final Log LOG = LogFactory.getLog(pathDistrUndirected.class);
...
//									System.out.println("Path Till Now:" + _modifiedMessage.toString());
						...
						/* filtered edge*/
							...
							for( IEdge<MapValue, LongWritable, LongWritable> edge: currentVertex.getOutEdges() ) {
								...
								if ( compareValuesUtil(edge.getValue().get(nextStep.property).toString(), nextStep.value.toString()) ) {
									IVertex<MapValue, MapValue, LongWritable, LongWritable> otherVertex = getSubgraph().getVertexById(edge.getSinkVertexId());
									StringBuilder _modifiedMessage = new StringBuilder("");
									_modifiedMessage.append(vertexMessageStep.message).append("-->E:").append(String.valueOf(edge.getEdgeId().get())).append("-->V:").append(String.valueOf(otherVertex.getVertexId().get()));
									if ( !otherVertex.isRemote() ) {
...
//								System.out.println("Traversing edges");
								IVertex<MapValue, MapValue, LongWritable, LongWritable> otherVertex = getSubgraph().getVertexById(edge.getSinkVertexId());
								long otherVertexId = otherVertex.getVertexId().get();
								StringBuilder _modifiedMessage = new StringBuilder("");
								_modifiedMessage.append(vertexMessageStep.message).append("<--E:").append(String.valueOf(edge.getEdgeId().get())).append("<--V:").append(String.valueOf(otherVertexId));
								if ( !otherVertex.isRemote() ) {
									/* TODO :add the correct value to list*/
					...
					if ( prevStep.direction == Direction.OUT ) {
						...
						if ( prevStep.property == null && prevStep.value == null ) {
							...
							for( IEdge<MapValue, LongWritable, LongWritable> edge: currentVertex.getOutEdges()) {
								...
								long otherVertexId = otherVertex.getVertexId().get();
								StringBuilder _modifiedMessage = new StringBuilder("");
								_modifiedMessage.append("V:").append(String.valueOf(otherVertexId)).append("-->E:").append(String.valueOf(edge.getEdgeId().get())).append("-->").append(vertexMessageStep.message);
								if ( !otherVertex.isRemote() ) {
									/* TODO :add the correct value to list*/
						...
						/* filtered edge*/
							...
							for( IEdge<MapValue, LongWritable, LongWritable> edge: currentVertex.getOutEdges()) {
								...
								//output(partition.getId(), subgraph.getId(), currentVertex.getId()+":"+subgraphProperties.getValue("relation"));
								if ( compareValuesUtil(edge.getValue().get(prevStep.property), prevStep.value) ) {
									long otherVertexId = otherVertex.getVertexId().get();
					...
					while(It.hasNext()){
						...
						// TODO: make cost not count in probability when no predicate on edge/vertex
						{
							...
							if ( currentStep.property == null )
								...
								if ( hueristics.vertexPredicateMap.get(currentStep.property.toLowerCase()).containsKey(currentStep.value.toString()) ){
									probability = hueristics.vertexPredicateMap.get(currentStep.property.toLowerCase()).get(currentStep.value.toString()).probability;
							...
							if(nextStep.direction == Direction.OUT){
								if ( currentStep.property == null) {
									...
									avgDeg = hueristics.vertexPredicateMap.get(currentStep.property.toLowerCase()).get(currentStep.value.toString()).avgOutDegree; 
									avgRemoteDeg = hueristics.vertexPredicateMap.get(currentStep.property.toLowerCase()).get(currentStep.value.toString()).avgRemoteOutDegree;
									//System.out.println("AVGDEG:" +avgDeg + "REMOTEAVGDEG:" + avgRemoteDeg);
								}	
							}else if(nextStep.direction == Direction.IN){
								if ( currentStep.property == null) {
									...
									avgDeg = hueristics.vertexPredicateMap.get(currentStep.property.toLowerCase()).get(currentStep.value.toString()).avgInDegree;
									avgRemoteDeg = hueristics.vertexPredicateMap.get(currentStep.property.toLowerCase()).get(currentStep.value.toString()).avgRemoteInDegree;
									//System.out.println("AVGDEG:" +avgDeg + "REMOTEAVGDEG:" + avgRemoteDeg);
								}		
							}
							...
							if(nextStep.property == null)
								...
								if ( hueristics.edgePredicateMap.get(nextStep.property.toLowerCase()).containsKey(nextStep.value.toString()) ) {
									vScanCost = eScanCost * hueristics.edgePredicateMap.get(nextStep.property.toLowerCase()).get(nextStep.value.toString()).probability;
									networkCost = getSubgraph().getSubgraphValue().networkCoeff * prevScanCost * probability * avgRemoteDeg * hueristics.edgePredicateMap.get(nextStep.property.toLowerCase()).get(nextStep.value.toString()).probability;
									resultSetNumber *= hueristics.edgePredicateMap.get(nextStep.property.toLowerCase()).get(nextStep.value.toString()).probability;
...
//	                                                                         System.out.println("AVGDEG:" +avgDeg + "REMOTEAVGDEG:" + avgRemoteDeg);
	                                                             ...
	                                                             if(nextStep.property == null){
	                                                                     vScanCost = eScanCost;
	                                                                     networkCost = getSubgraph().getSubgraphValue().networkCoeff * prevScanCost * probability * avgRemoteDeg;
//	                                                                     System.out
...
//	                                           System.out.println(pos+":"+"for:"+String.valueOf(totalCost));
	                                     ...
	                                     if ( getSubgraph().getSubgraphValue().queryCostHolder[pos] != -1 )
	                                     {
	                                             if(!initDone)
	                                            	 ...
	                                            	 getSubgraph().getSubgraphValue().queryCostHolder[pos] +=hueristics.numVertices*getSubgraph().getSubgraphValue().indexCoeff;

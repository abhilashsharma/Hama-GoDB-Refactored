...
public class pathDistrInt extends
AbstractSubgraphComputation<pathDistrIntSubgraphState, MapValue, MapValue, Text, IntWritable, IntWritable, LongWritable>
implements ISubgraphWrapup{
	
	public pathDistrInt(String initMsg) {
		// TODO Auto-generated constructor stub
		Arguments=initMsg;
	}
	
	public static final Log LOG = LogFactory.getLog(pathDistrInt.class);
    ...
    public VertexMessageSteps(long _queryId, int _vertexId, String _message,
        int _stepsTraversed, int _startVertexId, int _startStep,
        long _previousSubgraphId, int _previousPartitionId) {
      // TODO Auto-generated constructor stub
	...
	class Pair{
        ...
        public Pair(Integer _endVertex,Long _prevSubgraphId, Integer _prevPartitionId) {
		...
		class RecursivePathMaintained{
			...
			public boolean equals(Object obj){
				...
				return (this.direction==other.direction && this.startStep.intValue()==other.startStep.intValue() && this.startVertex.intValue()==other.startVertex.intValue() && this.path.equals(other.path));
			}
			
			
			public RecursivePathMaintained(Integer _startVertex,Integer _startStep, String _path, int _direction){
		 ...
		 */
		 class RecursivePathKey {
	        ...
	        public RecursivePathKey(long _queryID,int _step,boolean _direction,int _endVertex){
	        	this.queryID=_queryID;
	        	this.step=_step;
	        	this.direction=_direction;
	        	this.endVertex=_endVertex;	        	
		...
		class OutputPathKey{
			...
			public OutputPathKey(long _queryID,int _step,boolean _direction,int _startVertex){
				this.queryID=_queryID;
				this.step=_step;
				this.direction=_direction;
				this.startVertex=_startVertex;
...
//		System.out.println("RECEIVED JOIN MESSAGE:" +message);
		...
		Integer endVertexId = Integer.parseInt(split[2]);
...
//		System.out.println("OUTPUT SIZE:"+step.startVertexId+":"+step.message+":"+getSubgraph().getSubgraphValue().outputPathMaintainance.get(new OutputPathKey(step.queryId,step.startStep,d,step.startVertexId)).size());
		...
		for (Pair entry: getSubgraph().getSubgraphValue().outputPathMaintainance.get(new OutputPathKey(step.queryId,step.startStep,d,step.startVertexId) )){
			...
			getSubgraph().getSubgraphValue().outputList.add(new OutputMessageSteps(remoteM, new LongWritable(entry.prevSubgraphId.intValue())));
		...
		// STATIC ONE TIME PROCESSES
		{
			...
			if(getSuperstep() == 0){
				...
				if(getSubgraph().getSubgraphValue().InEdges==null){
				  ...
				  getSubgraph().getSubgraphValue().InEdges=new HashMap<Integer,HashMap<Integer,EdgeAttr>>();  
				time=System.currentTimeMillis();
			
				
				String m="";
				
				for(IVertex<MapValue, MapValue, IntWritable, IntWritable> sourceVertex:getSubgraph().getLocalVertices())
		        	for(IEdge<MapValue, IntWritable, IntWritable> edge : sourceVertex.getOutEdges()) {
		        		
		        		IVertex<MapValue, MapValue, IntWritable, IntWritable> sinkVertex=getSubgraph().getVertexById(edge.getSinkVertexId());
...
//	        				ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForEdge(edge.getId());
	    			   		EdgeAttr attr= new EdgeAttr("relation", "null"/*subgraphProperties.getValue("relation").toString()*/,edge.getEdgeId().get(),false,null);      				
	    			   		getSubgraph().getSubgraphValue().InEdges.put(sinkVertex.getVertexId().get(), new HashMap<Integer,EdgeAttr>());
	    			   		getSubgraph().getSubgraphValue().InEdges.get(sinkVertex.getVertexId().get()).put(sourceVertex.getVertexId().get(), attr);
	    			   		//System.out.println("Accumulation inedge for edge "+ edge.getId() + " Value " + subgraphProperties.getValue("relation").toString() );
	        		...
	        		//TODO: generalize this for all attributes
	        			IRemoteVertex<MapValue,MapValue,IntWritable,IntWritable,LongWritable> remoteVertex = (IRemoteVertex<MapValue, MapValue, IntWritable, IntWritable, LongWritable>)sinkVertex;
			...
			for (IMessage<LongWritable, Text> _message: messageList){
				...
				for(String subgraphMessage:SubgraphMessages){
					String[] values = subgraphMessage.split(Pattern.quote("|"));
		        	 ...
		        	 int Sink=Integer.parseInt(values[2]);
		        	 String[] attr_data=values[3].split(":");
		        	 if(getSubgraph().getSubgraphValue().InEdges.containsKey(Sink))
		        	  {
		        		 EdgeAttr attr= new EdgeAttr(attr_data[0],attr_data[1],Integer.parseInt(values[4]),true,Long.parseLong(values[5]));      				
		        		 getSubgraph().getSubgraphValue().InEdges.get(Sink).put(Source, attr);
		        	  }
		        	 else
		        	  {
		        		 EdgeAttr attr= new EdgeAttr(attr_data[0],attr_data[1],Integer.parseInt(values[4]),true,Long.parseLong(values[5]));   
		        		 getSubgraph().getSubgraphValue().InEdges.put(Sink, new HashMap<Integer,EdgeAttr>());
		        		 getSubgraph().getSubgraphValue().InEdges.get(Sink).put(Source, attr);
		        	  }
				}
						...
						if(hits.length>0){
							for (int i=0;i<hits.length;i++){
								...
								if ( Long.valueOf(doc.get("subgraphid")) == getSubgraph().getSubgraphId().get() ){
									Integer _vertexId = Integer.valueOf(doc.get("id"));
...
//					for(IVertex<MapWritable, MapWritable, IntWritable, IntWritable> vertex: getSubgraph().getLocalVertices()) {
...
//					System.out.println("Querying Output Path:" + vertexMessageStep.queryId+","+vertexMessageStep.startStep+","+true+","+vertexMessageStep.startVertexId );
				...
				IVertex<MapValue, MapValue, IntWritable, IntWritable> currentVertex = getSubgraph().getVertexById(new IntWritable(vertexMessageStep.vertexId.intValue()));
				...
				if( nextStep.type == Type.EDGE ) {
					...
					if ( nextStep.direction == Direction.OUT ) {
						...
						if ( nextStep.property == null && nextStep.value == null ) {
							for( IEdge<MapValue, IntWritable, IntWritable> edge: currentVertex.getOutEdges()) {
								//count++;
//								System.out.println("Traversing edges");
								IVertex<MapValue, MapValue, IntWritable, IntWritable> otherVertex = getSubgraph().getVertexById(edge.getSinkVertexId());
...
//									System.out.println("Path Till Now:" + _modifiedMessage.toString());
						...
						if ( nextStep.property == null && nextStep.value == null ) {
							...
							for(Entry<Integer, EdgeAttr> edgeMap: getSubgraph().getSubgraphValue().InEdges.get(currentVertex.getVertexId().get()).entrySet()) {
								int otherVertexId = edgeMap.getKey();
								StringBuilder _modifiedMessage = new StringBuilder("");
...
//					System.out.println("Querying Output Path:" + vertexMessageStep.queryId+","+vertexMessageStep.startStep+","+false+","+vertexMessageStep.startVertexId );
				...
				IVertex<MapValue, MapValue, IntWritable, IntWritable> currentVertex = getSubgraph().getVertexById(new IntWritable(vertexMessageStep.vertexId));
...
//						ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForVertex(currentVertex.getId());
			...
			for(VertexMessageSteps stuff: getSubgraph().getSubgraphValue().forwardRemoteVertexList){
				...
				IRemoteVertex<MapValue,MapValue,IntWritable,IntWritable,LongWritable> remoteVertex = (IRemoteVertex<MapValue, MapValue, IntWritable, IntWritable, LongWritable>)getSubgraph().getVertexById(new IntWritable(stuff.vertexId));
...
// Aggregating results
	...
	public void wrapup() { 
	  ...
	  LOG.info("Ending Query Execution");
		...
		for(Entry<Integer, ResultSet> entry: getSubgraph().getSubgraphValue().resultsMap.entrySet()) {

...
public class reachabilityDistrHist extends
AbstractSubgraphComputation<reachabilityDistrHistSubgraphState, MapValue, MapValue, Text, LongWritable, LongWritable, LongWritable> 
implements ISubgraphWrapup{
	
	
	public reachabilityDistrHist(String initMsg) {
		// TODO Auto-generated constructor stub
		Arguments=initMsg;
	}
	public static final Log LOG = LogFactory.getLog(reachabilityDistrHist.class);
	long time;
...
//				System.out.println("***********DONE**************");
          ...
          getSubgraph()
              .getSubgraphValue().InEdges = new HashMap<Long, HashMap<Long, EdgeAttr>>();

          for (IVertex<MapValue, MapValue, LongWritable, LongWritable> sourceVertex : getSubgraph()
              .getLocalVertices())
            for (IEdge<MapValue, LongWritable, LongWritable> edge : sourceVertex
                .getOutEdges()) {
              // if sink vertex is not remote then add inedge to appropriate
              // data structure, otherwise send source value to remote partition
              IVertex<MapValue, MapValue, LongWritable, LongWritable> sinkVertex = getSubgraph()
                  .getVertexById(edge.getSinkVertexId());
              if (!sinkVertex.isRemote()) {
                if (getSubgraph().getSubgraphValue().InEdges
                    .containsKey(sinkVertex.getVertexId().get())) {
                  if (!getSubgraph().getSubgraphValue().InEdges
                      .get(sinkVertex.getVertexId().get())
                      .containsKey(sourceVertex.getVertexId().get())) {

                    EdgeAttr attr = new EdgeAttr("relation",
                        "null"/*
                               * subgraphProperties.getValue("relation").
                               * toString()
                               */, edge.getEdgeId().get(), false, null);
                    getSubgraph().getSubgraphValue().InEdges
                        .get(sinkVertex.getVertexId().get())
                        .put(sourceVertex.getVertexId().get(), attr);
                    // System.out.println("Accumulation inedge for edge "+
                    // edge.getId() + " Value " +
                    // subgraphProperties.getValue("relation").toString() );
                  }
                  ...
                  EdgeAttr attr = new EdgeAttr("relation",
                      "null"/*
                             ...
                             * )
                             */, edge.getEdgeId().get(), false, null);
                  getSubgraph().getSubgraphValue().InEdges.put(
                      sinkVertex.getVertexId().get(),
                      new HashMap<Long, EdgeAttr>());
                  getSubgraph().getSubgraphValue().InEdges
                      .get(sinkVertex.getVertexId().get())
                      .put(sourceVertex.getVertexId().get(), attr);
                  // System.out.println("Accumulation inedge for edge "+
                ...
                // FIXME: partition id is not required in hama as existing bug
                // is fixed in this
                ...
                if (!getSubgraph().getSubgraphValue().MessagePacking
                    .containsKey(remoteVertex.getSubgraphId().get()))
                  getSubgraph().getSubgraphValue().MessagePacking.put(
                      remoteVertex.getSubgraphId().get(),
                      new StringBuilder("#|" + sourceVertex.getVertexId().get()
                          + "|" + remoteVertex.getVertexId().get() + "|"
                          + "relation" + ":"
                          + "null" /*
                                    * subgraphProperties.getValue("relation").
                                    * toString()
                                    */ + "|" + edge.getEdgeId().get() + "|"
                          + getSubgraph().getSubgraphId().get() + "|" + 0));
                else {
                  getSubgraph().getSubgraphValue().MessagePacking
                      .get(remoteVertex.getSubgraphId().get()).append("$")
                      .append("#|").append(sourceVertex.getVertexId().get())
                      .append("|").append(remoteVertex.getVertexId().get())
                      .append("|").append("relation").append(":")
                      .append("null" /*
                                      * subgraphProperties.getValue("relation").
                                      * toString()
                                      */).append("|").append(edge.getEdgeId())
                      .append("|").append(getSubgraph().getSubgraphId());
                  // append("|").append(partition.getId());

                }

                // TODO: generalize this for all attributes
                // String m ="#|" + edge.getSource().getId() + "|" +
                // edge.getSink().getId() + "|" + "relation" + ":" +
                // "null"/*subgraphProperties.getValue("relation").toString()*/+"|"
                // + edge.getId()+"|" + subgraph.getId() + "|" +
                // partition.getId();
                // SubGraphMessage msg = new SubGraphMessage(m.getBytes());
                // msg.setTargetSubgraph(edge.getSink().getRemoteSubgraphId());
                // sendMessage(msg);
                // System.out.println("Sending Message:" + new
                // String(msg.getData()));
              }
            }

          // FIXME: change type of subgraphid stored everywhere to LongWritable,
          // check overhead involved while using LongWritable
          for (Map.Entry<Long, StringBuilder> remoteSubgraphMessage : getSubgraph()
              .getSubgraphValue().MessagePacking.entrySet()) {
			...
			// "subgraphId/20:attr?21,12,23|attr?12,12"       This is an example message sent to Subgraphs of remote Sinks 
				...
				for (IMessage<LongWritable,Text> _message: messageList){
					...
					//FIXME: sinkpartitionid i.e. values[6] no longer received:DONE
				...
				getSubgraph().getSubgraphValue().hueristics=HistogramHueristicsLoad.getInstance();
                                  ...
                                  {       
                                          ...
                                          Double prevScanCost = getSubgraph().getSubgraphValue().hueristics.numVertices;
                                          {
                                                  
                                                  {
                                                          ...
                                                          if ( getSubgraph().getSubgraphValue().hueristics.probabilityOfVertex(currentStep.property, currentStep.value.toString())!=-1  ) 
                                                            probability =  getSubgraph().getSubgraphValue().hueristics.probabilityOfVertex(currentStep.property, currentStep.value.toString());
                                                          else {
                                                          ...
                                                          avgDeg=getSubgraph().getSubgraphValue().hueristics.avgDeg(currentStep.property, currentStep.value.toString(),true,true );
                                                          avgRemoteDeg=getSubgraph().getSubgraphValue().hueristics.avgRemoteDeg(currentStep.property, currentStep.value.toString(), true,true);
                                                          if (totalCost!=-1)
                                  ...
                                  //###########################reverse cost#######################################################
                                  {
                                          ...
                                          Double prevScanCost = getSubgraph().getSubgraphValue().hueristics.numVertices;
                                          {
                                                  
                                                  {
                                                          ...
                                                          avgDeg=getSubgraph().getSubgraphValue().hueristics.avgDeg(currentStep.property, currentStep.value.toString(),false,true );
                                                          avgRemoteDeg=getSubgraph().getSubgraphValue().hueristics.avgRemoteDeg(currentStep.property, currentStep.value.toString(), false,true);     
                                                          if (totalCost!=-1)
                                                                  totalCost = prevScanCost * probability *  ( avgDeg + (1+getSubgraph().getSubgraphValue().networkCoeff) * avgRemoteDeg);
                                                  }       
...
//								System.out.println("FOR:6");
				...
				while(!getSubgraph().getSubgraphValue().revLocalVertexList.isEmpty()) {
					...
					IVertex<MapValue,MapValue,LongWritable,LongWritable> currentVertex = getSubgraph().getVertexById(new LongWritable(vertexMessageStep.vertexId));
//					System.out.println("ANY Inedges +"+subgraph.getId()+":" + InEdges.containsKey(currentVertex.getId()) + " Contains:" + subgraph.containsVertex(currentVertex.getId()));
	  ...
	  LOG.info("Ending Query Execution");
	  }
          {
                  for(String partialForwardPath: getSubgraph().getSubgraphValue().resultsSet.forwardResultSet) {
                          System.out.println("ResultFORWARD : " + partialForwardPath);

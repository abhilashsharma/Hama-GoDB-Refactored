...
public class DoBFS extends
	...
	public static final Log LOG = LogFactory.getLog(DoBFS.class);
	...
	public void compute(Iterable<IMessage<LongWritable, BFSMessage>> messages) throws IOException {
		...
		while ((step = queue.poll()) != null) {
			...
			for (IEdge<MapValue, LongWritable, LongWritable> edge : currentVertex.getOutEdges()) {
				...
				if (!otherVertex.isRemote()) { // other vertex is local
				...
				} else { // other vertex is remote
					// NOTE: the other vertex has not yet been fully visited.
					// We cannot complete the visit of other vertex in this subgraph.
					// We send a traversal message, and at the remote subgraph, it has to
					// check if other vertex is visited. If not, add to results and if
					// depth condition met, resume traversal.
					...
					long remoteSGID =
					        ((IRemoteVertex<MapValue, MapValue, LongWritable, LongWritable, LongWritable>) otherVertex)
					                .getSubgraphId().get();
					TraversalWriter traversalMessage = remoteTraversalMap.get(remoteSGID);
					if (traversalMessage == null) {
						traversalMessage = new TraversalWriter();
						remoteTraversalMap.put(remoteSGID, traversalMessage);
					}
					// When this message reaches and that vertex is not visited, then it
					// will be added to the resultset, and traversal proceeds.
					// At that time we MUST check depth and then proceed with traversal.
					traversalMessage.addTraversal(step.rootSubgraph, step.rootVertex, step.targetVertex, edge.getEdgeId().get(),
					        otherVID, step.depth + 1);
				}
			}

...
import in.dream_lab.goffish.hama.succinctstructure.SuccinctArraySubgraphInEdges;
...
import in.dream_lab.goffish.hama.succinctstructure.SuccinctArrayVertexInEdges;
...
public class pathDistrSuccinctArrayStructureInEdges extends
AbstractSubgraphComputation<pathDistrSubgraphSuccinctArrayStructureStateInEdges, MapValue, MapValue, Text, LongWritable, LongWritable, LongWritable> 
implements ISubgraphWrapup{
	
	public pathDistrSuccinctArrayStructureInEdges(String initMsg) {
		// TODO Auto-generated constructor stub
		Arguments=initMsg;
	}
	
	public static final Log LOG = LogFactory.getLog(pathDistrSuccinctArrayStructureInEdges.class);
...
//		hueristics=HueristicsLoad.getInstance();//loading this at a different place
		...
		delimArray.add("|");
		 ...
		 SuccinctArraySubgraphInEdges sg=(SuccinctArraySubgraphInEdges)getSubgraph();
		...
		// STATIC ONE TIME PROCESSES
		{
			...
			if(getSuperstep() == 0){
				...
				{	
					...
					hueristics=HueristicsLoad.getInstance("/user/swapnil/gplusNew2PHeuristics/gplusNew2PHeuristics.ser");//TODO:change heuristics path for new Graph
...
//								LOG.info("Querying end");
					...
					System.out.println("Starting Vertices:" +hitList.size());	
						...
						if(hitList.size()>0){
							int pos = getSubgraph().getSubgraphValue().startPos;
...
//				flushAllLogs();
			...
			while(!getSubgraph().getSubgraphValue().forwardLocalVertexList.isEmpty()) {
				...
				SuccinctArrayVertexInEdges<MapValue,MapValue,LongWritable,LongWritable> currentVertex = new SuccinctArrayVertexInEdges(new LongWritable(vertexMessageStep.vertexId),sg.getVertexBufferList(),sg.getOutEdgeBufferList(),sg.getInEdgeBufferList());
...
//						ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForVertex(currentVertex.getId());
						if ( compareValuesUtil(String.valueOf(currentVertex.getPropforVertex(propToIndex.get(prevStep.property))), prevStep.value.toString()) ) {
							/* add appropriate value later*/

 ...
 */
package in.dream_lab.goffish.godb.bfs.succinct;
...
public class BFSDistrSuccinctArray extends
AbstractSubgraphComputation<BFSDistrSuccinctArraySubgraphState, MapValue, MapValue, Text, LongWritable, LongWritable, LongWritable> implements ISubgraphWrapup {
	
	 public static final Log LOG = LogFactory.getLog(BFSDistrSuccinctArray.class);
	public BFSDistrSuccinctArray(String initMsg) {
...
//								LOG.info("Querying end");
					...
					LOG.info("*******Querying done********:"+hitList);
...
//				IVertex<MapValue,MapValue,LongWritable,LongWritable> currentVertex = getSubgraph().getVertexById(new LongWritable(vertexMessageStep.vertexId));
				SplitPropertySuccinctArrayVertex<MapValue,MapValue,LongWritable,LongWritable> currentVertex = new SplitPropertySuccinctArrayVertex(new LongWritable(vertexMessageStep.vertexId),sg.getVertexBuffer(),sg.getEdgeBufferList(),sg.getPropertyBufferMap());
							...
							for( Long edge: edges.getFirst() ) {
								...
								_modifiedMessage.append(vertexMessageStep.message).append("-->E:").append("-->V:").append(String.valueOf(edge));
								
								
									getSubgraph().getSubgraphValue().forwardLocalVertexList.add(new VertexMessageSteps(edge,_modifiedMessage.toString(),vertexMessageStep.stepsTraversed+1, vertexMessageStep.startVertexId, vertexMessageStep.startSubgraphId, vertexMessageStep.startPartitionId));
							...
							for( Long edge: edges.getSecond() ) {
								...
								_modifiedMessage.append(vertexMessageStep.message).append("-->E:").append("-->V:").append(String.valueOf(edge));
									...
									getSubgraph().getSubgraphValue().forwardRemoteVertexList.add(new VertexMessageSteps(edge,_modifiedMessage.toString(),vertexMessageStep.stepsTraversed+1,vertexMessageStep.startVertexId, vertexMessageStep.startSubgraphId,vertexMessageStep.startPartitionId));
...
//				IRemoteVertex<MapValue,MapValue,LongWritable,LongWritable,LongWritable> remoteVertex = (IRemoteVertex)getSubgraph().getVertexById(new LongWritable(stuff.vertexId));//remoteVertex=null if remote inedge
				...
				//new LongWritable((long) sg.getRemoteMap().get(stuff.vertexId))

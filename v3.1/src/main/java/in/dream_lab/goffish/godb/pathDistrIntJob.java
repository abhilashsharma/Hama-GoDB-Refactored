...
import in.dream_lab.goffish.hama.IntMapPartitionSubsetGsonReader;
import in.dream_lab.goffish.hama.NonSplitTextInputFormat;

public class pathDistrIntJob {

	 public static void main(String args[]) throws IOException,InterruptedException, ClassNotFoundException, ParseException
	  {
		  HamaConfiguration conf = new HamaConfiguration();
		  GraphJob job = new GraphJob(conf, pathDistrInt2.class);
		  job.setJobName("Path with Intwritable Ids");
		  ...
		  job.setSubgraphValueClass(pathDistrInt2SubgraphState.class);
		  job.setInputFormat(NonSplitTextInputFormat.class);
		  job.setInputReaderClass(IntMapPartitionSubsetGsonReader.class);

...
public class pathDistrInt2 extends
AbstractSubgraphComputation<pathDistrInt2SubgraphState, MapValue, MapValue, Text, IntWritable, IntWritable, LongWritable>
implements ISubgraphWrapup{
	
	public pathDistrInt2(String initMsg) {
		// TODO Auto-generated constructor stub
		Arguments=initMsg;
	}
	
	public static final Log LOG = LogFactory.getLog(pathDistrInt2.class);
			...
			if(getSuperstep() == 0){
				...
				for(IVertex<MapValue, MapValue, IntWritable, IntWritable> sourceVertex:getSubgraph().getLocalVertices())
	        		...
	        		//if sink vertex is not remote then add inedge to appropriate data structure, otherwise send source value to remote partition
		        		Long sinkVertexId=new Long(sinkVertex.getVertexId().get());
		        		Long sourceVertexId=new Long(sourceVertex.getVertexId().get());
	        		if(!sinkVertex.isRemote())
	        		{
	        			if(getSubgraph().getSubgraphValue().InEdges.containsKey(sinkVertexId))
	        			{
	        			   	if(!getSubgraph().getSubgraphValue().InEdges.get(sinkVertexId).containsKey(sourceVertexId))
	        			   	{
...
//	        			   		ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForEdge(edge.getId());
	        			   		...
	        			   		getSubgraph().getSubgraphValue().InEdges.get(sinkVertexId).put(sourceVertexId, attr);
...
//	        				ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForEdge(edge.getId());
	    			   		EdgeAttr attr= new EdgeAttr("relation", "null"/*subgraphProperties.getValue("relation").toString()*/,edge.getEdgeId().get(),false,null);      				
	    			   		getSubgraph().getSubgraphValue().InEdges.put(sinkVertexId, new HashMap<Long,EdgeAttr>());
	    			   		getSubgraph().getSubgraphValue().InEdges.get(sinkVertexId).put(sourceVertexId, attr);
	    			   		//System.out.println("Accumulation inedge for edge "+ edge.getId() + " Value " + subgraphProperties.getValue("relation").toString() );
...
//								System.out.println("Traversing edges");
								...
								Long otherVertexId=new Long(edge.getSinkVertexId().get());
...
//						ISubgraphObjectProperties subgraphProperties = subgraphInstance.getPropertiesForVertex(currentVertex.getId());
			...
			for(VertexMessageSteps stuff: getSubgraph().getSubgraphValue().forwardRemoteVertexList){
				...
				IRemoteVertex<MapValue,MapValue,IntWritable,IntWritable,LongWritable> remoteVertex = (IRemoteVertex<MapValue, MapValue, IntWritable, IntWritable, LongWritable>)getSubgraph().getVertexById(new IntWritable(stuff.vertexId.intValue()));

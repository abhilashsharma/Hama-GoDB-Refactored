...
public class pathDistrSplitPropertySuccinctArrayStructure extends
AbstractSubgraphComputation<pathDistrSubgraphSplitPropertySuccinctArrayStructureState, MapValue, MapValue, Text, LongWritable, LongWritable, LongWritable> 
implements ISubgraphWrapup{
	
	public pathDistrSplitPropertySuccinctArrayStructure(String initMsg) {
		// TODO Auto-generated constructor stub
		Arguments=initMsg;
	}
	
	public static final Log LOG = LogFactory.getLog(pathDistrSplitPropertySuccinctArrayStructure.class);
...
//						LOG.info("Prop:" + prop + " Path: " + vertexMessageStep.message);
						if ( compareValuesUtil(prop, nextStep.value.toString()) ) {	/* add appropriate value later*/
							getSubgraph().getSubgraphValue().forwardLocalVertexList.add(new VertexMessageSteps(vertexMessageStep.queryId,vertexMessageStep.vertexId,vertexMessageStep.message,vertexMessageStep.stepsTraversed+1, vertexMessageStep.startVertexId,vertexMessageStep.startStep, vertexMessageStep.previousSubgraphId, vertexMessageStep.previousPartitionId));
							//forwardLocalVertexList.add(vertexMessageStep);
						}
					}

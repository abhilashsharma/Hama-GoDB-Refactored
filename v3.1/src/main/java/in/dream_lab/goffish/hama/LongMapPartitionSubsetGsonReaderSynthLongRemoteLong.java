...
public class LongMapPartitionSubsetGsonReaderSynthLongRemoteLong<S extends Writable, V extends Writable, E extends Writable, K extends Writable, M extends Writable>
    implements
  ...
  public LongMapPartitionSubsetGsonReaderSynthLongRemoteLong(
      BSPPeer<Writable, Writable, Writable, Writable, Message<K, M>> peer,
...
//    int remoteVertices = 0;
    for (IVertex<V, E, LongWritable, LongWritable> vertex : vertices) {
      for (IEdge<E, LongWritable, LongWritable> e : vertex.getOutEdges()) {
        ...
        if (sink == null) {
          sink = new LongRemoteVertex<V, E, LongWritable, LongWritable, LongWritable>(
              sinkID.get());
    ...
    
    {
		...
		try {
			reader = new BufferedReader(new FileReader(
					"/scratch/abhilash/gplusGraph/gplusNewSM16P.txt")); //+ pseudoPartId //TODO:Removing partition id for now
			String line = reader.readLine();
			while (line != null) {
				String[] data=line.trim().split(",");
				LongWritable sinkID = new LongWritable(Long.parseLong(data[0]));
		        long remoteSubgraphID = Long.parseLong(data[1]);

 ...
 */
public class SuccinctArrayVertex12In<V extends Writable, E extends Writable, I extends Writable, J extends Writable> implements IVertex<V, E, I, J> {
//	public static final Log LOG = LogFactory.getLog(SuccinctVertex.class);
	public static final Log LOG = LogFactory.getLog(SuccinctArrayVertex12In.class);
	private I vid;
    private List<SuccinctIndexedFileBuffer>  outEdgeBufferList;
    private List<SuccinctIndexedFileBuffer>  inEdgeBufferList;
...
//    private char delim;
    ...
    public SuccinctArrayVertex12In(I vid, SuccinctIndexedFileBuffer _vertexSuccinctBuffer,HashMap<String, SuccinctIndexedFileBuffer> propertySuccinctBufferMap, List<SuccinctIndexedFileBuffer> _outEdgeBufferList,List<SuccinctIndexedFileBuffer> _inEdgeBufferList)
    {
        this.vid = vid;
        this.setVertexSuccinctBuffer(_vertexSuccinctBuffer);
        this.setPropertySuccinctBufferMap(propertySuccinctBufferMap);
        this.outEdgeBufferList = _outEdgeBufferList;
        this.inEdgeBufferList = _inEdgeBufferList;
//        this.delim = delim;
...
//    	LOG.info("EBUFFER size:" + ebufferList.size());
    	...
    	int iteration=0;
    	for(SuccinctIndexedFileBuffer ebuf:outEdgeBufferList) {
    		iteration++;
...
//        	LOG.info("Returning null edge buffer");
        ...
        for (Integer rid : recordID)
        {
            ...
            int lCount = (int)tokens.getLong(1);
            for(int i=2; i < 2 + lCount ; i++) 
                localSinks.add(tokens.getLong(i));
            for(int i=2 + lCount; i < tokens.size(); i++) 
...
//    	LOG.info("EBUFFER size:" + ebufferList.size());
    	...
    	long countStart=System.nanoTime();
    	int iteration=0;
    	for(SuccinctIndexedFileBuffer ebuf:inEdgeBufferList) {
    		iteration++;
    		long count = ebuf.count(wholeQuery.getBytes());
    		if(count > 0) {
//    			LOG.info("FOUND COUNT to be:" +count);

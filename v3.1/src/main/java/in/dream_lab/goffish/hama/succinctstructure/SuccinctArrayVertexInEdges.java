 ...
 */
public class SuccinctArrayVertexInEdges<V extends Writable, E extends Writable, I extends Writable, J extends Writable> implements IVertex<V, E, I, J> {
//	public static final Log LOG = LogFactory.getLog(SuccinctVertex.class);
	public static final Log LOG = LogFactory.getLog(SuccinctArrayVertexInEdges.class);
	private I vid;
    private List<SuccinctIndexedFileBuffer> vbufferList, oebufferList, iebufferList;
    private char delim;
    public SuccinctArrayVertexInEdges(I vid, List<SuccinctIndexedFileBuffer> vbufferList, List<SuccinctIndexedFileBuffer> oebufferList,List<SuccinctIndexedFileBuffer> iebufferList)
    {
        this.vid = vid;
...
//    	LOG.info("EBUFFER size:" + ebufferList.size());
    	...
    	int iteration=0;
    	for(SuccinctIndexedFileBuffer ebuf:oebufferList) {
    		iteration++;
...
//    	LOG.info("EBUFFER size:" + ebufferList.size());
    	...
    	long countStart=System.nanoTime();
    	int iteration=0;
    	for(SuccinctIndexedFileBuffer ebuf:iebufferList) {
    		iteration++;
    		long count = ebuf.count(wholeQuery.getBytes());
    		if(count > 0) {
//    			LOG.info("FOUND COUNT to be:" +count);

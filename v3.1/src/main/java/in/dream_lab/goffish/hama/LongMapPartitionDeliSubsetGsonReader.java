...
public class LongMapPartitionDeliSubsetGsonReader<S extends Writable, V extends Writable, E extends Writable, K extends Writable, M extends Writable>
    implements
  ...
  public LongMapPartitionDeliSubsetGsonReader(
      BSPPeer<Writable, Writable, Writable, Writable, Message<K, M>> peer,
...
//    }
    ...
    for(int i=0;i<vprop.length-1;i++){
        ...
        if(vertexPropertySet.contains(map[0])){
          ...
          LOG.info("Error processing property:" + vprop[i] + " #VPROP:" + jsonMap);
        }
